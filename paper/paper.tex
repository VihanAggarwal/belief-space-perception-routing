...
cross-sequence $\kappa$ transfer, cross-trajectory generalization, an
...
temporal resampling). The fog benefit peaks exactly where the
reference config is borderline-feasible and shrinks to n.s.\ at
looser deadlines; sweeping $\kappa$ gives a monotonic response from
...
entirely on a \emph{different} sequence and transplanting it changes
the RQ-H reduction by at most $0.07$\,pp in four tested pairs, still
significant in all four: $\kappa$ behaves as a stable structural
property of the imposed regime, not per-trajectory overfit.
...
reductions in all tested imposed-coupling conditions and $\kappa$
transfers across sequences, but after correction and replication we
...
\textbf{What the paper does and does not establish.} We could not
...
cannot, with a falsification control at exactly zero -- and (ii) a
reusable, released evaluation pipeline (imposed-schedule substrate,
de-circularization, independent-workload probe, multiple-testing
protocol) that any deployment can run on its own traces to test
whether \emph{its} regime is coupled before adopting the method.
...
replication run, zero of 24 tests support naturally-occurring
coupling in 8 real sequences; $\kappa$ nonetheless transfers cleanly
across sequences; and the pseudo-GT accuracy axis is an internal
consistency check only, so the timing mechanism is established but
config optimality under real task accuracy is not. The result is a
