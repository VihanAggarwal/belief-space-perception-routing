...
present (significant over seeds in five of six conditions, and
pooled across 10 sequences over 6 heterogeneous conditions with a
trajectory-level CI excluding zero, sign test $p{=}0.001$), while
...
independent load proxies: after Benjamini--Hochberg correction and a
replication run, zero of 24 tests support naturally-occurring
coupling -- an honest null we report as the method's central open
premise, together with the released pipeline any deployment can use
to test its own traces. Deadline-miss
...
reporting null findings plainly; and (iv) directly interrogates the
coupling assumption itself with five checks -- measured coupling via
three independent load proxies under a multiple-testing correction,
cross-sequence transfer of the full belief model (HMM and $\kappa$), an
oracle/reactive-baseline bound, and a real-GT accuracy-axis
validation -- rather than only exhibiting the mechanism under a
constructed regime. We release a fully reproducible, config-driven
pipeline.\footnotemark[1]
...
\cite{wang2021notallimages}, driving-specific early-exiting
...
estimated jointly with sensor fault. PRAM-R \cite{pramr2025},
adaptive fusion \cite{mees2016choosing}, and DynMM \cite{dynmm} gate
modalities or route via an LLM without modeling compute contention.
To our knowledge, no prior work couples a temporally-smoothed
sensor-fault belief to a
...
pseudo-GT frontier gives a consistent internal ordering (C1 highest,
C4 lowest) and the agreement gap traded against timeliness
($0.24$--$0.53$ F1, C1 vs.\ C4) \emph{under the pseudo-GT metric}; the
deadline-miss numbers below do not depend on this metric at all.
...
averaged over all frames) is nonnegative throughout (significantly
higher for off-road, rain, night; fog break-even, $+0.07$\,pp).
...
state knowledge does not by itself tighten. A \textbf{Reactive-Latency}
baseline (no sensor information at all) achieves a lower miss rate in
several conditions but at an accuracy cost (pseudo-GT): the combined
utility $U$ is lower than Joint's in five of seven conditions
(significantly on full off-road and fog) -- miss rate alone favors the
naive baseline exactly where it sacrifices the accuracy coupling is
meant to protect. Router overhead is negligible:
...
driven by a handful of frames (all six exclude zero under temporal
resampling). The fog benefit peaks where the reference config is
borderline-feasible and shrinks to n.s.\ at looser deadlines;
sweeping $\kappa$ gives a monotonic response, $0.0$\,pp at $\kappa{=}0$
to $+10.9$\,pp at $\kappa{=}1$ (Fig.~\ref{fig:phase}: exactly zero at
$\kappa{=}0$ on all seven tracks). A denser nine-config frontier
leaves the fog result intact ($+6.0$ vs.\ $+9.4$\,pp).
...
sequences each) the per-sequence effect is positive in all six traces
and the cluster bootstrap stays positive, but the cross-sequence
$t$-interval includes zero for both -- \emph{directionally
consistent}, not generalizing \emph{within a condition}. Pooling all
10 sequences \emph{across} 6 conditions is a distinct, weaker claim
that does clear the bar: mean $+3.72$\,pp, $t$-CI $[1.94,5.50]$,
cluster bootstrap $[2.41,5.43]$, sign test $10/10$ positive
($p{=}0.0010$) -- generalization across heterogeneous conditions,
not yet confirmed within any single condition at $n{=}3$.

\textbf{(c) Cross-sequence transfer of the full belief model.}
Fitting the \emph{entire} fault-belief estimator (per-channel
switching HMMs) \emph{and} $\kappa$ on one sequence and testing on a
held-out one, the transferred detector still recovers faults (bAcc
$0.65$--$0.90$ vs.\ $0.73$--$0.95$ native) and RQ-H stays positive in
all 8 tested pairs, significant in 5 -- including cross-\emph{condition}
transfers (rain$\rightarrow$fog $+7.8$\,pp, fog$\rightarrow$fog
$+17.9$\,pp). The whole belief model, not just $\kappa$, generalizes
across trajectories.
...
(b) Cross-seq.\ generalization & Within-condition ($n{=}3$): $t$-CI incl.\ 0. Pooled across 10 seq./6 conditions: $t$-CI $[1.94,5.50]$, sign test $p{=}0.001$ \\
...
reductions in all tested imposed-coupling conditions, generalizes
across 10 sequences pooled over 6 heterogeneous conditions
($p{=}0.001$), and its belief model transfers across sequences -- but
after correction and replication we find \emph{no} confirmed case of
naturally-occurring coupling in un-manipulated RADIATE data. RQ-H is
a proof of mechanism under a controlled, experimentally-imposed
regime -- not evidence that such coupling, or the accuracy
differences it is scored against, arise naturally in the field.
...
fault-free trajectory. The benefit tracks fault \emph{severity}
(deadline-bindingness) more than frequency or correlation, an
interpretation inferred from condition-level outcomes. Since the
threshold rule matches the noisy-OR, the supported claim is that
\emph{using fault belief to anticipate contention} helps -- not that
the particular fusion algorithm is itself the advance. We could not
demonstrate the coupling premise in ordinary data (zero of 24
corrected tests): the contributions are (i) a proof of mechanism --
when coupling exists, a router that exploits it beats one that
cannot -- and (ii) a reusable, released evaluation pipeline
(imposed-schedule substrate, de-circularization, independent-workload
probe, multiple-testing protocol) that any deployment can run on its
own traces to test whether \emph{its} regime is coupled.
...
frames, so seed CIs measure noise sensitivity, not generalization --
which we separately bootstrap within-condition (n=3,
non-$t$-significant) and pooled across conditions (n=10,
significant). RADIATE frames are rectified left-camera only;
WoodScape's temporal axis is dataset order, not continuous video.
Absolute latencies are dev-environment (Apple M5/MPS); a Jetson-class
study remains the natural next step.

\textbf{Future work.} A factorial/cross-coupled HMM would upgrade
coupled-belief fusion to true joint estimation. Most importantly: more
sequences and repeated \texttt{rain\_2\_0} trials to power the
corrected real-coupling test; real embedded contention on a Jetson;
re-profiling RQ-H on real RADIATE annotations; and a closed-loop
downstream-task evaluation measuring task success.
...
coupling is enabled (significant in five of six conditions, and
pooled across 10 sequences over 6 conditions with $p{=}0.001$), while
...
coupling regime: after correction and replication, zero of 24 tests
support naturally-occurring coupling in 8 real sequences; the full
belief model (HMM and $\kappa$) nonetheless transfers across
sequences, significant in five of eight held-out pairs including
cross-condition ones; and the pseudo-GT accuracy axis is an internal
consistency check only, so the timing mechanism is established but
config optimality under real accuracy is not. The result is a
proof of mechanism under controlled, imposed coupling, with a clearly
scoped open question -- whether such coupling arises naturally -- and
a reproducible pipeline built to let any deployment answer that
question on its own traces.
